...
abstract class Functions
{
    ...
    public static function dateTimeDB(?string $string): string|bool
    {
        ...
        return self::formatDateString(str_replace("/", "-", $string), 'Y-m-d H:i:s');
    }

    /**
     * Constrói uma data a partir de string e a formata, devolvendo false quando
     * a string não é uma data válida.
     *
     * O construtor de DateTimeImmutable lança em vez de retornar false, então a
     * conversão precisa do try/catch para honrar o contrato destes helpers.
     */
    private static function formatDateString(string $value, string $format): string|bool
    {
        if ($value === '') {
            return false;
        }
    ...
    public static function dateTimeBR(string $string): string|bool
    {
        ...
        return self::formatDateString(str_replace("/", "-", $string), 'd/m/Y H:i:s');
    }
    ...
    {
        return self::formatDateString($string, 'Y-m-d');
    }
    ...
    {
        return self::formatDateString($string, 'd/m/Y');
    }
     ...
     */
    public static function formatIpAddress($ip): string|bool
    {
     ...
     *
     * Cabeçalhos como X-Forwarded-For são preenchidos pelo cliente e só são
     * considerados quando o REMOTE_ADDR consta em TRUSTED_PROXIES. Sem isso,
     * qualquer um forja o próprio IP e escapa de rate limit e auditoria.
     *
    ...
    public static function getUserIp(): string
    {
        ...
        foreach (['HTTP_X_FORWARDED_FOR', 'HTTP_CLIENT_IP'] as $header) {
            ...
            $clientIps = explode(',', (string) $_SERVER[$header]);
            $candidate = trim((string) array_shift($clientIps));
